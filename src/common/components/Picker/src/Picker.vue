...
<script>
  ...
  export default {
    ...
    created() {
      this.lineHeight = this.$utils.h2px(this.lineHeight)
      this.reset()
      // this.list = list
    },
    computed: {
    ...
    methods: {
      ...
      reset() {
        ...
        this.translateZ = this.$utils.dp2rem(this.r()) + 'rem'
      },
      sty(i) {
      ...
      inertia(start, position, target) {
        if (start <= target) {
          ...
          console.log(this.finger.currentMove, i, '选中的')
        ...
        let move = (position * start * (1000 / 60)) + (0.5 * this.a * (1000 / 60) * (1000 / 60)) + this.finger.currentMove
