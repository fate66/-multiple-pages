...
<script>
  ...
  export default {
    ...
    computed: {
      linePont() {
        console.log(this.sumWidth, this.lineRig, '---')
        return ((this.sumWidth - this.lineRig) / this.sumWidth * 100) + '%'
      },
      scrolSty() {
        return {
          width: this.$utils.widthpx2rem(this.sumWidth) + 'rem',
          height: this.$utils.heightpx2rem(this.lineHeight) + 'rem'
        }
      },
      autoWidth() {
        let f = !this.tabWidth
      ...
      lineItemSty() {
        return {
          width: this.$utils.widthpx2rem(this.lineWidth) + 'rem',
      ...
      lineSty() {
        ...
        if (this.average) {
          ...
          width = (this.tabs[this.selIndex] && this.tabs[this.selIndex].tabWidth) || this.tabWidth
          width == 0 && (this.$refs[`tabItem${this.selIndex}`] && (width = this.$utils.px2w(this.$refs[`tabItem${this.selIndex}`][0].$el.offsetWidth)))
        }
        return {
          width: this.$utils.widthpx2rem(width) + 'rem',
          right: this.linePont,
          height: this.$utils.heightpx2rem(this.lineHeight) + 'rem',
      ...
      lineRig() {
        if (this.average) {
          ...
          this.tabs.forEach((item, i) => {
            if (i <= this.selIndex) {
              w += this.autoWidth ? this.$utils.px2w(this.$refs[`tabItem${i}`][0].$el.offsetWidth) : item.tabWidth || this.tabWidth
              w += item.tabMarginLeft || this.tabMarginLeft
            }
          })
          return w
    ...
    methods: {
      ...
      tabDomW() {
        ...
        for (let i = 0; i < this.tabs.length; i++) {
          w += this.$utils.px2w(this.$refs[`tabItem${i}`][0].$el.offsetWidth)
          w += this.tabs[i].tabMarginLeft || this.tabMarginLeft
        }
        this.innerSumWidth = w
        this.showLine = true
        console.log(this.innerSumWidth, 'this._sumWidth')
      },
      tabUd() {
